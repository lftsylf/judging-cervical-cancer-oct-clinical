#!/usr/bin/env bash
# Ablation：关闭 Aux；WMA + EMA；Clinical=1，CORAL=0
...
echo "======================================"
echo "Ablation: NO AUX (WMA+EMA) | CORAL=0 | Clinical=1"
echo "  医院: ${HOSPITALS[*]}"
echo "  Seeds: ${SEEDS[*]}"
echo "  总运行数: ${TOTAL_RUNS}"
echo "  USE_WMA=1 | ENABLE_EMA=1 | ENABLE_AUX=0 | BATCH=${OPTIGENESIS_BATCH_SIZE} | EPOCHS=${MAX_EPOCHS}"
echo "  输出根: ${PROJECT_ROOT}/outputs_ablation_no_aux/"
echo "======================================"
...
for HOSP in "${HOSPITALS[@]}"; do
  ...
  for SEED in "${SEEDS[@]}"; do
    ...
    echo "# 模式: WMA=1 | EMA=1 | AUX=0 | Clinical=1 | CORAL=0"
    echo "# 输出: outputs_ablation_no_aux/${HOSP}/seed_${SEED}/"
    echo "##############################################"

    export HOSPITAL_NAME="$HOSP"
    export OPTIGENESIS_SEED="$SEED"
    export OPTIGENESIS_OUTPUT_DIR="$PROJECT_ROOT/outputs_ablation_no_aux/${HOSP}"
...
echo ""
echo "======================================"
echo "Ablation NO AUX 全部结束: ${PROJECT_ROOT}/outputs_ablation_no_aux"
echo "======================================"
