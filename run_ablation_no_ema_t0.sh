#!/usr/bin/env bash
# Ablation：关闭 EMA；WMA + Aux；Clinical=1，CORAL=0
...
echo "======================================"
echo "Ablation: NO EMA (WMA+Aux) | CORAL=0 | Clinical=1"
echo "  医院: ${HOSPITALS[*]}"
echo "  Seeds: ${SEEDS[*]}"
echo "  总运行数: ${TOTAL_RUNS}"
echo "  USE_WMA=1 | ENABLE_EMA=0 | ENABLE_AUX=1 | BATCH=${OPTIGENESIS_BATCH_SIZE} | EPOCHS=${MAX_EPOCHS}"
echo "  输出根: ${PROJECT_ROOT}/outputs_ablation_no_ema/"
echo "======================================"
...
for HOSP in "${HOSPITALS[@]}"; do
  ...
  for SEED in "${SEEDS[@]}"; do
    ...
    echo "# 模式: WMA=1 | EMA=0 | AUX=1 | Clinical=1 | CORAL=0"
    echo "# 输出: outputs_ablation_no_ema/${HOSP}/seed_${SEED}/"
    echo "##############################################"

    export HOSPITAL_NAME="$HOSP"
    export OPTIGENESIS_SEED="$SEED"
    export OPTIGENESIS_OUTPUT_DIR="$PROJECT_ROOT/outputs_ablation_no_ema/${HOSP}"
...
echo ""
echo "======================================"
echo "Ablation NO EMA 全部结束: ${PROJECT_ROOT}/outputs_ablation_no_ema"
echo "======================================"
