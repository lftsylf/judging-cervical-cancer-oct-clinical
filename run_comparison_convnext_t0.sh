#!/usr/bin/env bash
# ConvNeXt-Small T0 对比：标准多模态（Focal+EDL），关闭 WMA / EMA / AUX / CORAL。
# 3 折 × 5 seeds，30 epoch，batch=2。输出根目录：outputs_comparison_convnext_small/
set -euo pipefail
...
export OPTIGENESIS_BACKBONE="convnext_small"
...
for HOSP in "${HOSPITALS[@]}"; do
  for SEED in "${SEEDS[@]}"; do
    echo "--------------------------------------"
    echo "ConvNeXt-Small | ${HOSP} | seed=${SEED}"
    echo "--------------------------------------"

    export HOSPITAL_NAME="$HOSP"
    export OPTIGENESIS_SEED="$SEED"
    export OPTIGENESIS_EPOCHS="$MAX_EPOCHS"
    export OPTIGENESIS_OUTPUT_DIR="$PROJECT_ROOT/outputs_comparison_convnext_small/$HOSP"
...
echo "======================================"
echo "ConvNeXt-Small 对比全部完成: ${PROJECT_ROOT}/outputs_comparison_convnext_small"
echo "======================================"
