...
MAX_EPOCHS=30
BASELINE_OUT_ROOT="${BASELINE_OUT_ROOT:-$PROJECT_ROOT/outputs/paper_v4/baseline/ours_uw_agg_t0}"
...
echo "======================================"
echo "Paper v4 | Ours T0 | 帧级 EDL + uncertainty-weighted aggregation | OCT-only"
echo "骨干=${CAPTURE_BACKBONE} | LR=${CAPTURE_LR} | POS_WEIGHT=${CAPTURE_POS_WEIGHT}"
echo "FRAME_AGG=uncertainty_weighted | τ=${CAPTURE_AGG_TEMP} | USE_CLINICAL=0 | BATCH=4 | EPOCHS<=${MAX_EPOCHS}"
...
for HOSP in "${HOSPITALS[@]}"; do
  for SEED in "${SEEDS[@]}"; do
    RUN_IDX=$((RUN_IDX + 1))
    echo "--------------------------------------"
    echo "Ours T0 [${RUN_IDX}/${TOTAL_RUNS}] | ${HOSP} | seed=${SEED}"
    echo "--------------------------------------"
