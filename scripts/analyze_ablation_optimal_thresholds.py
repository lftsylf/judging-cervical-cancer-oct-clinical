#!/usr/bin/env python3
"""
对三个补充消融目录执行统一的 Youden Index 阈值解耦分析，并输出表B汇总。
...
输出文件:
  - outputs_ablation_no_wma_summary.csv
  - outputs_ablation_no_ema_summary.csv
  - outputs_ablation_no_aux_summary.csv
"""
...
ABLATION_DIR_TO_SUMMARY = {
    "outputs_ablation_no_wma": "outputs_ablation_no_wma_summary.csv",
    "outputs_ablation_no_ema": "outputs_ablation_no_ema_summary.csv",
    "outputs_ablation_no_aux": "outputs_ablation_no_aux_summary.csv",
}
...
def _iter_external_csvs(ablation_dir: str):
...
def analyze_one_ablation(ablation_dir: str) -> pd.DataFrame:
    rows: List[dict] = []
    for hosp, seed, path in _iter_external_csvs(ablation_dir):
        df = pd.read_csv(path)
    ...
    if not rows:
        raise ValueError(f"{ablation_dir} 未找到 external_sample_predictions.csv")
...
def main() -> None:
    for ablation_dir, summary_name in ABLATION_DIR_TO_SUMMARY.items():
        summary_df = analyze_one_ablation(ablation_dir)
        out_path = os.path.join(PROJECT_ROOT, summary_name)
        summary_df.to_csv(out_path, index=False, encoding="utf-8-sig")
        print(f"[OK] {ablation_dir} -> {out_path}")
