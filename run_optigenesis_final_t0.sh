#!/usr/bin/env bash
# OptiGenesis 完全体验证：WMA + 临床多模态 + CORAL 域适应
# 3 中心 × 5 seeds，最多 30 epoch（main.py 验证集 AUC Early Stopping，patience=10 不变）
set -euo pipefail
...
# 8GB 显存：多模态 + CORAL 协方差，Batch 4 易 OOM，固定为 2
...
echo "======================================"
echo "OptiGenesis 完全体 (T0) 开始 — WMA + CORAL + Clinical"
echo "  医院: ${HOSPITALS[*]}"
echo "  Seeds: ${SEEDS[*]}"
...
echo "  OPTIGENESIS_ENABLE_CORAL=1（Config.ENABLE_DOMAIN_CORAL）"
echo "  OPTIGENESIS_USE_CLINICAL=1（多模态临床特征）"
echo "  OPTIGENESIS_BATCH_SIZE=${OPTIGENESIS_BATCH_SIZE}"
echo "  输出根目录: ${PROJECT_ROOT}/outputs_optigenesis_final/<hospital>/seed_<seed>/"
echo "======================================"
...
for HOSP in "${HOSPITALS[@]}"; do
  ...
  for SEED in "${SEEDS[@]}"; do
    ...
    echo "# ── 本 Seed 模式: WMA=${OPTIGENESIS_USE_WMA} | CORAL(域适应)=${OPTIGENESIS_ENABLE_CORAL} | Clinical=${OPTIGENESIS_USE_CLINICAL} ──"
    echo "#即将启动: python main.py"
    echo "# 输出: outputs_optigenesis_final/${HOSP}/seed_${SEED}/"
    echo "##############################################"

    export HOSPITAL_NAME="$HOSP"
    export OPTIGENESIS_SEED="$SEED"
    export OPTIGENESIS_OUTPUT_DIR="$PROJECT_ROOT/outputs_optigenesis_final/${HOSP}"
...
echo ""
echo "======================================"
echo "OptiGenesis 完全体 (T0) 全部结束。"
echo "结果根目录: ${PROJECT_ROOT}/outputs_optigenesis_final"
echo "======================================"
