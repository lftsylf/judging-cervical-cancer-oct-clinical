#!/usr/bin/env bash
# OptiGenesis 完全体 V2：WMA + Clinical + EMA + Aux，关闭 CORAL（防 OOM）
...
echo "======================================"
echo "OptiGenesis 完全体 V2 (T0) 开始 — WMA + Clinical + EMA + Aux | CORAL=0"
echo "  医院: ${HOSPITALS[*]}"
echo "  Seeds: ${SEEDS[*]}"
echo "  总运行数: ${TOTAL_RUNS} (${#HOSPITALS[@]} × ${#SEEDS[@]})"
echo "  OPTIGENESIS_EPOCHS=${MAX_EPOCHS}（Early Stopping patience=10，见 main.py）"
echo "  OPTIGENESIS_USE_WMA=1 | OPTIGENESIS_USE_CLINICAL=1"
echo "  OPTIGENESIS_ENABLE_EMA=1 | OPTIGENESIS_ENABLE_AUX=1"
echo "  OPTIGENESIS_ENABLE_CORAL=0"
echo "  OPTIGENESIS_BATCH_SIZE=${OPTIGENESIS_BATCH_SIZE}"
echo "  输出根目录: ${PROJECT_ROOT}/outputs_optigenesis_v2/<hospital>/seed_<seed>/"
echo "======================================"
...
for HOSP in "${HOSPITALS[@]}"; do
  ...
  for SEED in "${SEEDS[@]}"; do
    ...
    echo "# ── 模式: WMA=1 | Clinical=1 | EMA=1 | AUX=1 | CORAL=0 (完全体V2 最终决战) ──"
    echo "#即将启动: python main.py"
    echo "# 输出: outputs_optigenesis_v2/${HOSP}/seed_${SEED}/"
    echo "##############################################"

    export HOSPITAL_NAME="$HOSP"
    export OPTIGENESIS_SEED="$SEED"
    export OPTIGENESIS_OUTPUT_DIR="$PROJECT_ROOT/outputs_optigenesis_v2/${HOSP}"
...
echo ""
echo "======================================"
echo "OptiGenesis 完全体 V2 (T0) 全部结束。"
echo "结果根目录: ${PROJECT_ROOT}/outputs_optigenesis_v2"
echo "======================================"
